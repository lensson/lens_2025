...
import com.lens.common.web.utils.CookieUtils;
...
public class JwtAuthenticationTokenFilter extends OncePerRequestFilter {
    ...
    protected void doFilterInternal(HttpServletRequest request, HttpServletResponse response, FilterChain chain)
        ...
        if (authHeader != null && authHeader.startsWith(tokenHead)) {
            ...
            // 获取在线的管理员信息
            String onlineAdmin = redisUtil.get(RedisConstants.LOGIN_TOKEN_KEY + RedisConstants.SEGMENTATION + authHeader);
            if (StringUtils.isNotEmpty(onlineAdmin) && !jwtTokenUtil.isExpiration(token, base64Secret)) {
                ...
                if (survivalSecond < refreshSecond) {
                    ...
                    redisUtil.delete(RedisConstants.LOGIN_TOKEN_KEY + Constants.SYMBOL_COLON + authHeader);
                    redisUtil.delete(RedisConstants.LOGIN_UUID_KEY + Constants.SYMBOL_COLON + oldTokenUid);
                    // 将新token赋值，用于后续使用
                    authHeader = newToken;

                    // 将新的Token存入Redis中
                    redisUtil.setEx(RedisConstants.LOGIN_TOKEN_KEY + Constants.SYMBOL_COLON + newToken, JsonUtils.objectToJson(newOnlineAdmin), expiresSecond, TimeUnit.SECONDS);
                    // 维护 uuid - token 互相转换的Redis集合【主要用于在线用户管理】
                    redisUtil.setEx(RedisConstants.LOGIN_UUID_KEY + Constants.SYMBOL_COLON + tokenUid, newToken, expiresSecond, TimeUnit.SECONDS);
                }
            } else {
            ...
            request.setAttribute(SysConstants.ADMIN_UID, adminUid);
            request.setAttribute(SysConstants.USER_NAME, username);
            request.setAttribute(SysConstants.TOKEN, authHeader);
