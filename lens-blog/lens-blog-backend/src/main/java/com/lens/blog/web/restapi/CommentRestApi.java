...
public class CommentRestApi {
    ...
    public String getListByUser(HttpServletRequest request, @Validated({GetList.class}) @RequestBody UserVO userVO) {
        ...
        String requestUserUid = request.getAttribute(SysConstants.USER_UID).toString();
        QueryWrapper<Comment> queryWrapper = new QueryWrapper<>();
        ...
        queryWrapper.orderByDesc(SQLConstants.CREATE_TIME);
        // 查找出 我的评论 和 我的回复
        queryWrapper.and(wrapper -> wrapper.eq(SQLConstants.USER_UID, requestUserUid).or().eq(SQLConstants.TO_USER_UID, requestUserUid));
        IPage<Comment> pageList = commentService.page(page, queryWrapper);
        List<Comment> list = pageList.getRecords();
        List<String> userUidList = new ArrayList<>();
        list.forEach(item -> {
        ...
        resultMap.put(SysConstants.COMMENT_LIST, commentList);
        resultMap.put(SysConstants.REPLY_LIST, replyList);
    ...
    public String getPraiseListByUser(@ApiParam(name = "currentPage", value = "当前页数", required = false) @RequestParam(name = "currentPage", required = false, defaultValue = "1") Long currentPage,
        ...
        queryWrappe.eq(SQLConstants.USER_UID, userUid);
        queryWrappe.eq(SQLConstants.TYPE, ECommentType.PRAISE);
        queryWrappe.eq(SQLConstants.STATUS, EStatus.ENABLE);
        queryWrappe.orderByDesc(SQLConstants.CREATE_TIME);
        Page<Comment> page = new Page<>();
    ...
    public String add(@Validated({Insert.class}) @RequestBody CommentVO commentVO, BindingResult result) {
        ...
        // 判断是否开启全局评论功能
        if (SysConstants.CAN_NOT_COMMENT.equals(webConfig.getOpenComment())) {
            return ResultUtil.result(SysConstants.ERROR, MessageConstants.NO_COMMENTS_OPEN);
        }
        // 判断该博客是否开启评论功能
        if (StringUtils.isNotEmpty(commentVO.getBlogUid())) {
            Blog blog = blogService.getById(commentVO.getBlogUid());
            if (SysConstants.CAN_NOT_COMMENT.equals(blog.getOpenComment())) {
                return ResultUtil.result(SysConstants.ERROR, MessageConstants.BLOG_NO_OPEN_COMMENTS);
            }
        }
        String userUid = request.getAttribute(SysConstants.USER_UID).toString();
        User user = userService.getById(userUid);
        // 判断字数是否超过限制
        if (commentVO.getContent().length() > SysConstants.ONE_ZERO_TWO_FOUR) {
            return ResultUtil.result(SysConstants.ERROR, MessageConstants.COMMENT_CAN_NOT_MORE_THAN_1024);
        }
        // 判断该用户是否被禁言
        if (user.getCommentStatus() == SysConstants.ZERO) {
            return ResultUtil.result(SysConstants.ERROR, MessageConstants.YOU_DONT_HAVE_PERMISSION_TO_SPEAK);
        }
        // 判断是否发送过多无意义评论
        String jsonResult = redisUtil.get(RedisConstants.USER_PUBLISH_SPAM_COMMENT_COUNT + BaseSysConstants.REDIS_SEGMENTATION + userUid);
        if (!StringUtils.isEmpty(jsonResult)) {
            Integer count = Integer.valueOf(jsonResult);
            if (count >= Constants.NUM_FIVE) {
                return ResultUtil.result(SysConstants.ERROR, MessageConstants.PLEASE_TRY_AGAIN_IN_AN_HOUR);
            }
        }
        // 判断是否垃圾评论
        ...
        if (StringUtils.isCommentSpam(content)) {
            if (StringUtils.isEmpty(jsonResult)) {
                Integer count = 0;
                redisUtil.setEx(RedisConstants.USER_PUBLISH_SPAM_COMMENT_COUNT + BaseSysConstants.REDIS_SEGMENTATION + userUid, count.toString(), 1, TimeUnit.HOURS);
            } else {
                redisUtil.incrBy(RedisConstants.USER_PUBLISH_SPAM_COMMENT_COUNT + BaseSysConstants.REDIS_SEGMENTATION + userUid, 1);
            }
            return ResultUtil.result(SysConstants.ERROR, MessageConstants.COMMENT_IS_SPAM);
        }
        ...
        if (StringUtils.isNotEmpty(commentVO.getToUserUid())) {
            ...
            if (toUser.getStartEmailNotification() == SysConstants.ONE) {
                ...
                if (toComment != null && StringUtils.isNotEmpty(toComment.getContent())) {
                    ...
                    map.put(SysConstants.EMAIL, toUser.getEmail());
                    map.put(SysConstants.TEXT, commentVO.getContent());
                    map.put(SysConstants.TO_TEXT, toComment.getContent());
                    map.put(SysConstants.NICKNAME, user.getNickName());
                    map.put(SysConstants.TO_NICKNAME, toUser.getNickName());
                    map.put(SysConstants.USER_UID, toUser.getUid());
                    // 获取评论跳转的链接
        ...
        if (StringUtils.isNotEmpty(comment.getToUserUid())) {
            String redisKey = RedisConstants.USER_RECEIVE_COMMENT_COUNT + Constants.SYMBOL_COLON + comment.getToUserUid();
        ...
        return ResultUtil.result(SysConstants.SUCCESS, comment);
    }
    ...
    public String reportComment(HttpServletRequest request, @Validated({GetOne.class}) @RequestBody CommentVO commentVO, BindingResult result) {
        ...
        if (comment == null || comment.getStatus() == EStatus.DISABLED) {
            return ResultUtil.result(SysConstants.ERROR, MessageConstants.COMMENT_IS_NOT_EXIST);
        }

        // 判断举报的评论是否是自己的
        if (comment.getUserUid().equals(commentVO.getUserUid())) {
            return ResultUtil.result(SysConstants.ERROR, MessageConstants.CAN_NOT_REPORT_YOURSELF_COMMENTS);
        }
        // 查看该用户是否重复举报该评论
        QueryWrapper<CommentReport> queryWrapper = new QueryWrapper<>();
        queryWrapper.eq(SQLConstants.USER_UID, commentVO.getUserUid());
        queryWrapper.eq(SQLConstants.REPORT_COMMENT_UID, comment.getUid());
        List<CommentReport> commentReportList = commentReportService.list(queryWrapper);
        if (commentReportList.size() > 0) {
            return ResultUtil.result(SysConstants.ERROR, MessageConstants.CAN_NOT_REPEAT_REPORT_COMMENT);
        }
    ...
    public String deleteBatch(HttpServletRequest request, @Validated({Delete.class}) @RequestBody CommentVO commentVO, BindingResult result) {
        ...
        // 获取该评论一级评论下所有的子评论
        QueryWrapper<Comment> queryWrapper = new QueryWrapper<>();
        queryWrapper.eq(SQLConstants.FIRST_COMMENT_UID, firstCommentUid);
        queryWrapper.eq(SQLConstants.STATUS, EStatus.ENABLE);
    ...
    public String bindUserEmail(@PathVariable("userUid") String userUid) {
        ...
        User user = userService.getById(userUid);
        if (user == null) {
            ResultUtil.result(SysConstants.ERROR, MessageConstants.OPERATION_FAIL);
        }
        ...
        if (StringUtils.isNotEmpty(user.getValidCode())) {
            ...
            String userInfo = redisUtil.get(RedisConstants.USER_TOKEN + Constants.SYMBOL_COLON + accessToken);
            if (StringUtils.isNotEmpty(userInfo)) {
                Map<String, Object> map = JsonUtils.jsonToMap(userInfo);
                // 关闭邮件通知
                map.put(SysConstants.START_EMAIL_NOTIFICATION, 0);
                redisUtil.setEx(RedisConstants.USER_TOKEN + Constants.SYMBOL_COLON + accessToken, JsonUtils.objectToJson(map), userTokenSurvivalTime, TimeUnit.HOURS);
            }
        }
    ...
    public String getUserReceiveCommentCount(HttpServletRequest request) {
        ...
        if (request.getAttribute(SysConstants.USER_UID) != null) {
            String userUid = request.getAttribute(SysConstants.USER_UID).toString();
            String redisKey = RedisConstants.USER_RECEIVE_COMMENT_COUNT + Constants.SYMBOL_COLON + userUid;
            String count = redisUtil.get(redisKey);
            if (StringUtils.isNotEmpty(count)) {
