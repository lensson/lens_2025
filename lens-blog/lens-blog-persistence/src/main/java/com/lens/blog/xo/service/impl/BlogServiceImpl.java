...
public class BlogServiceImpl extends SuperServiceImpl<BlogMapper, Blog> implements BlogService {
    ...
    public List<Blog> setTagAndSortByBlogList(List<Blog> list) {
        ...
        list.forEach(item -> {
            ...
            if (StringUtils.isNotEmpty(item.getTagUid())) {
                List<String> tagUidsTemp = StringUtils.changeStringToString(item.getTagUid(), BaseSysConstants.FILE_SEGMENTATION);
    ...
    public Blog setTagByBlog(Blog blog) {
        ...
        if (!StringUtils.isEmpty(tagUid)) {
            String[] uids = tagUid.split(SysConstants.FILE_SEGMENTATION);
    ...
    public List<Blog> getBlogListByLevel(Integer level) {
        ...
        queryWrapper.eq(BaseSQLConstants.LEVEL, level);
        queryWrapper.eq(BaseSQLConstants.IS_PUBLISH, EPublish.PUBLISH);
    ...
    public IPage<Blog> getBlogPageByLevel(Page<Blog> page, Integer level, Integer useSort) {
        ...
        if (useSort == 0) {
            queryWrapper.orderByDesc(BaseSQLConstants.CREATE_TIME);
        } else {
            queryWrapper.orderByDesc(BaseSQLConstants.SORT);
        }

        //因为首页并不需要显示内容，所以需要排除掉内容字段
        queryWrapper.select(Blog.class, i -> !i.getProperty().equals(SysConstants.CONTENT));
    ...
    public List<Map<String, Object>> getBlogCountByTag() {
        // 从Redis中获取标签下包含的博客数量
        String jsonArrayList = redisUtil.get(RedisConstants.DASHBOARD + Constants.SYMBOL_COLON + RedisConstants.BLOG_COUNT_BY_TAG);
        ...
        for (Map<String, Object> item : blogCoutByTagMap) {
            String tagUid = String.valueOf(item.get(SQLConstants.TAG_UID));
            // java.lang.Number是Integer,Long的父类
            Number num = (Number) item.get(SysConstants.COUNT);
        ...
        for (Map.Entry<String, Integer> entry : tagMap.entrySet()) {
            ...
            if (tagEntityMap.get(tagUid) != null) {
                ...
                itemResultMap.put(SysConstants.TAG_UID, tagUid);
                itemResultMap.put(SysConstants.NAME, tagName);
                itemResultMap.put(SysConstants.VALUE, count);
                resultList.add(itemResultMap);
            }
        }
        // 将 每个标签下文章数目 存入到Redis【过期时间2小时】
        if (resultList.size() > 0) {
            redisUtil.setEx(RedisConstants.DASHBOARD + Constants.SYMBOL_COLON + RedisConstants.BLOG_COUNT_BY_TAG, JsonUtils.objectToJson(resultList), 2, TimeUnit.HOURS);
        }
        return resultList;
    }
    ...
    public List<Map<String, Object>> getBlogCountByBlogSort() {
        ...
        String jsonArrayList = redisUtil.get(RedisConstants.DASHBOARD + Constants.SYMBOL_COLON + RedisConstants.BLOG_COUNT_BY_SORT);
        if (StringUtils.isNotEmpty(jsonArrayList)) {
            ArrayList jsonList = JsonUtils.jsonArrayToArrayList(jsonArrayList);
            return jsonList;
        }
        ...
        for (Map<String, Object> item : blogCoutByBlogSortMap) {
            ...
            String blogSortUid = String.valueOf(item.get(SQLConstants.BLOG_SORT_UID));
            // java.lang.Number是Integer,Long的父类
            Number num = (Number) item.get(SysConstants.COUNT);
            Integer count = 0;
            if (num != null) {
        ...
        for (Map.Entry<String, Integer> entry : blogSortMap.entrySet()) {
            ...
            if (blogSortEntityMap.get(blogSortUid) != null) {
                ...
                Integer count = entry.getValue();
                Map<String, Object> itemResultMap = new HashMap<>();
                itemResultMap.put(SysConstants.BLOG_SORT_UID, blogSortUid);
                itemResultMap.put(SysConstants.NAME, blogSortName);
                itemResultMap.put(SysConstants.VALUE, count);
                resultList.add(itemResultMap);
            }
        }
        ...
        if (resultList.size() > 0) {
            redisUtil.setEx(RedisConstants.DASHBOARD + Constants.SYMBOL_COLON + RedisConstants.BLOG_COUNT_BY_SORT, JsonUtils.objectToJson(resultList), 2, TimeUnit.HOURS);
        }
        return resultList;
    }
    ...
    public Map<String, Object> getBlogContributeCount() {
        // 从Redis中获取博客分类下包含的博客数量
        String jsonMap = redisUtil.get(RedisConstants.DASHBOARD + Constants.SYMBOL_COLON + RedisConstants.BLOG_CONTRIBUTE_COUNT);
        ...
        resultMap.put(SysConstants.CONTRIBUTE_DATE, contributeDateList);
        resultMap.put(SysConstants.BLOG_CONTRIBUTE_COUNT, resultList);
        // 将 全年博客贡献度 存入到Redis【过期时间2小时】
        redisUtil.setEx(RedisConstants.DASHBOARD + Constants.SYMBOL_COLON + RedisConstants.BLOG_CONTRIBUTE_COUNT, JsonUtils.objectToJson(resultMap), 2, TimeUnit.HOURS);
        return resultMap;
    }
    ...
    public IPage<Blog> getPageList(BlogVO blogVO) {
        ...
        if (StringUtils.isNotEmpty(blogVO.getKeyword()) && !StringUtils.isEmpty(blogVO.getKeyword().trim())) {
            queryWrapper.like(SQLConstants.TITLE, blogVO.getKeyword().trim());
        }
        if (!StringUtils.isEmpty(blogVO.getTagUid())) {
            queryWrapper.like(SQLConstants.TAG_UID, blogVO.getTagUid());
        }
        if (!StringUtils.isEmpty(blogVO.getBlogSortUid())) {
            queryWrapper.like(SQLConstants.BLOG_SORT_UID, blogVO.getBlogSortUid());
        }
        if (!StringUtils.isEmpty(blogVO.getLevelKeyword())) {
            queryWrapper.eq(SQLConstants.LEVEL, blogVO.getLevelKeyword());
        }
        if (!StringUtils.isEmpty(blogVO.getIsPublish())) {
            queryWrapper.eq(SQLConstants.IS_PUBLISH, blogVO.getIsPublish());
        }
        if (!StringUtils.isEmpty(blogVO.getIsOriginal())) {
            queryWrapper.eq(SQLConstants.IS_ORIGINAL, blogVO.getIsOriginal());
        }
        if(!StringUtils.isEmpty(blogVO.getType())) {
            queryWrapper.eq(SQLConstants.TYPE, blogVO.getType());
        }
        ...
        list.forEach(item -> {
            ...
            if (StringUtils.isNotEmpty(item.getTagUid())) {
                List<String> tagUidsTemp = StringUtils.changeStringToString(item.getTagUid(), SysConstants.FILE_SEGMENTATION);
                for (String itemTagUid : tagUidsTemp) {
                    tagUids.add(itemTagUid);
                }
            }
    ...
    public String addBlog(BlogVO blogVO) {
        ...
        QueryWrapper<Blog> queryWrapper = new QueryWrapper<>();
        queryWrapper.eq(SQLConstants.LEVEL, blogVO.getLevel());
        queryWrapper.eq(SQLConstants.STATUS, EStatus.ENABLE);
        ...
        Blog blog = new Blog();
        //如果是原创，作者为用户的昵称
        String projectName = sysParamsService.getSysParamsValueByKey(SysConstants.PROJECT_NAME_);
    ...
    public String deleteBlog(BlogVO blogVO) {
        ...
        if (save) {
            ...
            map.put(SysConstants.COMMAND, SysConstants.DELETE);
            map.put(SysConstants.BLOG_UID, blog.getUid());
            map.put(SysConstants.LEVEL, blog.getLevel());
            map.put(SysConstants.CREATE_TIME, blog.getCreateTime());
            //发送到RabbitMq
            rabbitTemplate.convertAndSend(SysConstants.EXCHANGE_DIRECT, SysConstants.MOGU_BLOG, map);
    ...
    public String deleteBatchBlog(List<BlogVO> blogVoList) {
        ...
        blogVoList.forEach(item -> {
            ...
            uidSbf.append(item.getUid() + SysConstants.FILE_SEGMENTATION);
        });
        ...
        if (save) {
            Map<String, Object> map = new HashMap<>();
            map.put(SysConstants.COMMAND, SysConstants.DELETE_BATCH);
            map.put(SysConstants.UID, uidSbf);
            //发送到RabbitMq
            rabbitTemplate.convertAndSend(SysConstants.EXCHANGE_DIRECT, SysConstants.MOGU_BLOG, map);
            // 移除所有包含该博客的专题Item
    ...
    public String uploadLocalBlog(List<MultipartFile> filedatas) {
        ...
        if (systemConfig == null) {
            return ResultUtil.errorWithMessage(MessageConstants.SYSTEM_CONFIG_NOT_EXIST);
        } else {
            if (EOpenStatus.OPEN.equals(systemConfig.getUploadQiNiu()) && (StringUtils.isEmpty(systemConfig.getQiNiuPictureBaseUrl()) || StringUtils.isEmpty(systemConfig.getQiNiuAccessKey())
                ...
                return ResultUtil.errorWithMessage(MessageConstants.PLEASE_SET_QI_NIU);
            }

            if (EOpenStatus.OPEN.equals(systemConfig.getUploadLocal()) && StringUtils.isEmpty(systemConfig.getLocalPictureBaseUrl())) {
                return ResultUtil.errorWithMessage(MessageConstants.PLEASE_SET_LOCAL);
            }
        }
        ...
        HttpServletRequest request = RequestHolder.getRequest();
        String pictureList = request.getParameter(SysConstants.PICTURE_LIST);
        ...
        for (LinkedTreeMap<String, String> item : list) {
            ...
            if (EFilePriority.QI_NIU.equals(systemConfig.getContentPicturePriority())) {
                // 获取七牛云上的图片
                pictureMap.put(item.get(SysConstants.FILE_OLD_NAME), item.get(SysConstants.QI_NIU_URL));
            } else if(EFilePriority.LOCAL.equals(systemConfig.getContentPicturePriority())) {
                // 获取本地的图片
                pictureMap.put(item.get(SysConstants.FILE_OLD_NAME), item.get(SysConstants.PIC_URL));
            } else if(EFilePriority.MINIO.equals(systemConfig.getContentPicturePriority())) {
                // 获取MINIO的图片
                pictureMap.put(item.get(SysConstants.FILE_OLD_NAME), item.get(SysConstants.MINIO_URL));
            }
        }
        // 需要替换的图片Map
        ...
        for (String blogContent : fileContentList) {
            ...
            for (String matchStr : matchList) {
                ...
                if (splitList.length >= 5) {
                    ...
                    // 判断是网络图片还是本地图片
                    if (!pictureUrl.startsWith(SysConstants.HTTP)) {
                        // 那么就需要遍历全部的map和他匹配
    ...
    public void deleteRedisByBlogSort() {
        // 删除Redis中博客分类下的博客数量
        redisUtil.delete(RedisConstants.DASHBOARD + Constants.SYMBOL_COLON + RedisConstants.BLOG_COUNT_BY_SORT);
        // 删除博客相关缓存
        deleteRedisByBlog();
    }
    ...
    public void deleteRedisByBlogTag() {
        // 删除Redis中博客分类下的博客数量
        redisUtil.delete(RedisConstants.DASHBOARD + Constants.SYMBOL_COLON + RedisConstants.BLOG_COUNT_BY_TAG);
        // 删除博客相关缓存
        deleteRedisByBlog();
    }
    ...
    public void deleteRedisByBlog() {
        // 删除博客相关缓存
        redisUtil.delete(RedisConstants.NEW_BLOG);
        redisUtil.delete(RedisConstants.HOT_BLOG);
        redisUtil.delete(RedisConstants.BLOG_LEVEL + Constants.SYMBOL_COLON + ELevel.FIRST);
        redisUtil.delete(RedisConstants.BLOG_LEVEL + Constants.SYMBOL_COLON + ELevel.SECOND);
        redisUtil.delete(RedisConstants.BLOG_LEVEL + Constants.SYMBOL_COLON + ELevel.THIRD);
        redisUtil.delete(RedisConstants.BLOG_LEVEL + Constants.SYMBOL_COLON + ELevel.FOURTH);
    }
    ...
    public IPage<Blog> getBlogPageByLevel(Integer level, Long currentPage, Integer useSort) {
        ...
        //从Redis中获取内容
        String jsonResult = redisUtil.get(RedisConstants.BLOG_LEVEL + RedisConstants.SEGMENTATION + level);
        ...
        switch (level) {
            case ELevel.NORMAL: {
                blogCount = sysParamsService.getSysParamsValueByKey(SysConstants.BLOG_NEW_COUNT);
            }
            break;
            case ELevel.FIRST: {
                blogCount = sysParamsService.getSysParamsValueByKey(SysConstants.BLOG_FIRST_COUNT);
            }
            break;
            case ELevel.SECOND: {
                blogCount = sysParamsService.getSysParamsValueByKey(SysConstants.BLOG_SECOND_COUNT);
            }
            break;
            case ELevel.THIRD: {
                blogCount = sysParamsService.getSysParamsValueByKey(SysConstants.BLOG_THIRD_COUNT);
            }
            break;
            case ELevel.FOURTH: {
                blogCount = sysParamsService.getSysParamsValueByKey(SysConstants.BLOG_FOURTH_COUNT);
            }
            break;
        }
        if (StringUtils.isEmpty(blogCount)) {
            log.error(MessageConstants.PLEASE_CONFIGURE_SYSTEM_PARAMS);
        } else {
        ...
        if ((level == SysConstants.ONE || level == SysConstants.TWO) && list.size() == 0) {
            QueryWrapper<Blog> queryWrapper = new QueryWrapper<>();
            Page<Blog> hotPage = new Page<>();
            hotPage.setCurrent(1);
            String blogHotCount = sysParamsService.getSysParamsValueByKey(SysConstants.BLOG_HOT_COUNT);
            String blogSecondCount = sysParamsService.getSysParamsValueByKey(SysConstants.BLOG_SECOND_COUNT);
            ...
            if (firstBlogList.size() > 0) {
                redisUtil.setEx(RedisConstants.BLOG_LEVEL + Constants.SYMBOL_COLON + Constants.NUM_ONE, JsonUtils.objectToJson(firstBlogList), 1, TimeUnit.HOURS);
            }
            if (secondBlogList.size() > 0) {
                redisUtil.setEx(RedisConstants.BLOG_LEVEL + Constants.SYMBOL_COLON + Constants.NUM_TWO, JsonUtils.objectToJson(secondBlogList), 1, TimeUnit.HOURS);
            }

            switch (level) {
                case SysConstants.ONE: {
                    pageList.setRecords(firstBlogList);
                }
                break;
                case SysConstants.TWO: {
        ...
        if (list.size() > 0) {
            redisUtil.setEx(SysConstants.BLOG_LEVEL + SysConstants.REDIS_SEGMENTATION + level, JsonUtils.objectToJson(list).toString(), 1, TimeUnit.HOURS);
        }
        return pageList;
    }
    ...
    public IPage<Blog> getHotBlog() {
        //从Redis中获取内容
        String jsonResult = redisUtil.get(RedisConstants.HOT_BLOG);
        //判断redis中是否有文章
        if (StringUtils.isNotEmpty(jsonResult)) {
            List jsonResult2List = JsonUtils.jsonArrayToArrayList(jsonResult);
            IPage pageList = new Page();
        ...
        if (list.size() > 0) {
            redisUtil.setEx(RedisConstants.HOT_BLOG, JsonUtils.objectToJson(list), 1, TimeUnit.HOURS);
        }
        return pageList;
    }
...
//        if (currentPage == 1L) {
//            //从Redis中获取内容
//            String jsonResult = redisUtil.get(RedisConstants.NEW_BLOG);
//            //判断redis中是否有文章
//            if (StringUtils.isNotEmpty(jsonResult)) {
...
//        }
        ...
        //将从最新博客缓存到redis中
//        if (currentPage == 1L) {
//            redisUtil.setEx(RedisConstants.NEW_BLOG, JsonUtils.objectToJson(pageList), 1, TimeUnit.HOURS);
//        }
        return pageList;
    }
    ...
    public Integer getBlogPraiseCountByUid(String uid) {
        ...
        //从Redis取出用户点赞数据
        String pariseJsonResult = redisUtil.get(RedisConstants.BLOG_PRAISE + RedisConstants.SEGMENTATION + uid);
    ...
    public String praiseBlogByUid(String uid) {
        ...
        if (request.getAttribute(SysConstants.USER_UID) != null) {
            ...
            if (praise != null) {
                return ResultUtil.errorWithMessage(MessageConstants.YOU_HAVE_BEEN_PRISE);
            }
        } else {
            return ResultUtil.errorWithMessage(MessageConstants.PLEASE_LOGIN_TO_PRISE);
        }
        Blog blog = blogService.getById(uid);
        String pariseJsonResult = redisUtil.get(RedisConstants.BLOG_PRAISE + RedisConstants.SEGMENTATION + uid);
        if (StringUtils.isEmpty(pariseJsonResult)) {
            //给该博客点赞数
            redisUtil.set(RedisConstants.BLOG_PRAISE + RedisConstants.SEGMENTATION + uid, "1");
            ...
            //给该博客点赞 +1
            redisUtil.set(RedisConstants.BLOG_PRAISE + RedisConstants.SEGMENTATION + uid, count.toString());
    ...
    public IPage<Blog> getSameBlogByTagUid(String tagUid) {
        ...
        page.setCurrent(1);
        page.setSize(10);
        queryWrapper.like(SQLConstants.TAG_UID, tagUid);
        queryWrapper.orderByDesc(SQLConstants.CREATE_TIME);
        queryWrapper.eq(SQLConstants.STATUS, EStatus.ENABLE);
        queryWrapper.eq(SQLConstants.IS_PUBLISH, EPublish.PUBLISH);
        IPage<Blog> pageList = blogService.page(page, queryWrapper);
    ...
    public Map<String, Object> getBlogByKeyword(String keywords, Long currentPage, Long pageSize) {
        ...
        QueryWrapper<Blog> queryWrapper = new QueryWrapper<>();
        queryWrapper.and(wrapper -> wrapper.like(SQLConstants.TITLE, keyword).or().like(SQLConstants.SUMMARY, keyword));
        queryWrapper.eq(SQLConstants.STATUS, EStatus.ENABLE);
        queryWrapper.eq(SQLConstants.IS_PUBLISH, EPublish.PUBLISH);
        queryWrapper.select(Blog.class, i -> !i.getProperty().equals(SQLConstants.CONTENT));
        queryWrapper.orderByDesc(SQLConstants.CLICK_COUNT);
        ...
        Map<String, Object> map = new HashMap<>();
        // 返回总记录数
        map.put(SysConstants.TOTAL, iPage.getTotal());
        // 返回总页数
        map.put(SysConstants.TOTAL_PAGE, iPage.getPages());
        // 返回当前页大小
        map.put(SysConstants.PAGE_SIZE, pageSize);
        // 返回当前页
        map.put(SysConstants.CURRENT_PAGE, iPage.getCurrent());
        // 返回数据
        map.put(SysConstants.BLOG_LIST, blogList);
        return map;
    }
    ...
    public IPage<Blog> searchBlogByTag(String tagUid, Long currentPage, Long pageSize) {
        ...
        if (tag != null) {
            ...
            String jsonResult = redisUtil.get(RedisConstants.TAG_CLICK + RedisConstants.SEGMENTATION + ip + "#" + tagUid);
            if (StringUtils.isEmpty(jsonResult)) {
                ...
                redisUtil.setEx(RedisConstants.TAG_CLICK + RedisConstants.SEGMENTATION + ip + RedisConstants.WELL_NUMBER + tagUid, clickCount + "",
                        24, TimeUnit.HOURS);
            }
        }
        QueryWrapper<Blog> queryWrapper = new QueryWrapper<>();
    ...
    public IPage<Blog> searchBlogByBlogSort(String blogSortUid, Long currentPage, Long pageSize) {
        ...
        if (blogSort != null) {
            ...
            String jsonResult = redisUtil.get(RedisConstants.TAG_CLICK + RedisConstants.SEGMENTATION + ip + RedisConstants.WELL_NUMBER + blogSortUid);
            if (StringUtils.isEmpty(jsonResult)) {
                //给标签点击数增加
                ...
                blogSort.updateById();
                //将该用户点击记录存储到redis中, 24小时后过期
                redisUtil.setEx(RedisConstants.TAG_CLICK + RedisConstants.SEGMENTATION + ip + RedisConstants.WELL_NUMBER + blogSortUid, clickCount + "",
                        24, TimeUnit.HOURS);
            }
        }
    ...
    public IPage<Blog> searchBlogByAuthor(String author, Long currentPage, Long pageSize) {
        ...
        page.setCurrent(currentPage);
        page.setSize(pageSize);
        queryWrapper.eq(SQLConstants.AUTHOR, author);
        queryWrapper.eq(BaseSQLConstants.IS_PUBLISH, EPublish.PUBLISH);
        queryWrapper.eq(BaseSQLConstants.STATUS, EStatus.ENABLE);
        queryWrapper.orderByDesc(SQLConstants.CREATE_TIME);
        queryWrapper.select(Blog.class, i -> !i.getProperty().equals(SysConstants.CONTENT));
    ...
    public String getBlogTimeSortList() {
        //从Redis中获取内容
        String monthResult = redisUtil.get(SysConstants.MONTH_SET);
        //判断redis中时候包含归档的内容
        ...
        map.forEach((key, value) -> {
            redisUtil.set(SysConstants.BLOG_SORT_BY_MONTH + SysConstants.REDIS_SEGMENTATION + key, JsonUtils.objectToJson(value).toString());
        });

        //将从数据库查询的数据缓存到redis中
        redisUtil.set(SysConstants.MONTH_SET, JsonUtils.objectToJson(monthSet).toString());
    ...
    public String getArticleByMonth(String monthDate) {
        ...
        //从Redis中获取内容
        String contentResult = redisUtil.get(SysConstants.BLOG_SORT_BY_MONTH + SysConstants.REDIS_SEGMENTATION + monthDate);
        ...
        //将从数据库查询的数据缓存到redis中
        redisUtil.set(SysConstants.MONTH_SET, JsonUtils.objectToJson(monthSet));
    ...
    private String addVerdict(Long count, Integer level) {
        ...
        switch (level) {
            case ELevel.FIRST: {
                Long blogFirstCount = Long.valueOf(sysParamsService.getSysParamsValueByKey(SysConstants.BLOG_FIRST_COUNT));
            ...
            case ELevel.SECOND: {
                Long blogSecondCount = Long.valueOf(sysParamsService.getSysParamsValueByKey(SysConstants.BLOG_SECOND_COUNT));
            ...
            case ELevel.THIRD: {
                Long blogThirdCount = Long.valueOf(sysParamsService.getSysParamsValueByKey(SysConstants.BLOG_THIRD_COUNT));
            ...
            case ELevel.FOURTH: {
                Long blogFourthCount = Long.valueOf(sysParamsService.getSysParamsValueByKey(SysConstants.BLOG_FOURTH_COUNT));
    ...
    private void updateSolrAndRedis(Boolean isSave, Blog blog) {
        ...
        if (isSave && EPublish.PUBLISH.equals(blog.getIsPublish())) {
            Map<String, Object> map = new HashMap<>();
            map.put(SysConstants.COMMAND, SysConstants.ADD);
            map.put(SysConstants.BLOG_UID, blog.getUid());
            map.put(SysConstants.LEVEL, blog.getLevel());
            map.put(SysConstants.CREATE_TIME, blog.getCreateTime());
            ...
            Map<String, Object> map = new HashMap<>();
            map.put(SysConstants.COMMAND, SysConstants.EDIT);
            map.put(SysConstants.BLOG_UID, blog.getUid());
            map.put(SysConstants.LEVEL, blog.getLevel());
            map.put(SysConstants.CREATE_TIME, blog.getCreateTime());
