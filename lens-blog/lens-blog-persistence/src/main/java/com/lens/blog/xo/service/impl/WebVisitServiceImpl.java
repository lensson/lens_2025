...
public class WebVisitServiceImpl extends SuperServiceImpl<WebVisitMapper, WebVisit> implements WebVisitService {
    ...
    public void addWebVisit(String userUid, HttpServletRequest request, String behavior, String moduleUid, String otherData) {
        ...
        if (StringUtils.isEmpty(jsonResult)) {
            ...
            if (StringUtils.isNotEmpty(addresses)) {
                ...
                stringRedisTemplate.opsForValue().set("IP_SOURCE" + BaseSysConstants.REDIS_SEGMENTATION + ip, addresses, 24, TimeUnit.HOURS);
            }
        } else {
    ...
    public Map<String, Object> getVisitByWeek() {
        // 从Redis中获取一周访问量
        String weekVisitJson = redisUtil.get(RedisConstants.DASHBOARD + Constants.SYMBOL_COLON + RedisConstants.WEEK_VISIT);
        ...
        redisUtil.setEx(RedisConstants.DASHBOARD + Constants.SYMBOL_COLON + RedisConstants.WEEK_VISIT, JsonUtils.objectToJson(resultMap), 10, TimeUnit.MINUTES);
        return resultMap;
    }
    ...
    public IPage<WebVisit> getPageList(WebVisitVO webVisitVO) {
        ...
        if (!StringUtils.isEmpty(webVisitVO.getKeyword()) && !StringUtils.isEmpty(webVisitVO.getKeyword().trim())) {
            ...
            queryWrapper.like(SQLConstants.IP, webVisitVO.getKeyword().trim()).or().eq(SQLConstants.BEHAVIOR, behavior);
        }

        // 设置起始时间段
        if (!StringUtils.isEmpty(webVisitVO.getStartTime())) {
            String[] time = webVisitVO.getStartTime().split(SysConstants.FILE_SEGMENTATION);
            if (time.length == 2) {
        ...
        if (blogOids.size() > 0) {
            ...
            blogQueryWrapper.in(SQLConstants.OID, blogOids);
